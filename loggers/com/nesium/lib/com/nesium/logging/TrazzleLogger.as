package com.nesium.lib.com.nesium.logging {
	import com.nesium.lib.com.nesium.remoting.DuplexGateway;
	...
	public class TrazzleLogger extends EventDispatcher {
		...
		public function log(msg : String, stackIndex : uint = 4) : void {
			if (!g_stage) {
				throwNotInitedError();
				return;
			}
		...
		public function logBitmapData(bmp : BitmapData) : void {
			...
			g_gateway.invokeRemoteService('LoggingService', 'logPNG_width_height', PNGEncoder.encode(bmp), bmp.width, bmp.height);
		}
			...
			if (observers && (index = observers.indexOf(callback)) != -1) {
				if (remove) {
					...
					if (observers.length == 0) {
						g_gateway.invokeRemoteService('FileObservingService', 'stopObservingFile', path);
		...
		public function inspectObject(obj : *) : void {
			...
			g_gateway.invokeRemoteService('InspectionService', 'inspectObject_metadata', obj, describeType(obj));
		}
...
g_instance = new TrazzleLogger();
		...
		public static function throwNotInitedError() : void {
			...
			throw new Error('TrazzleLogger not inited. Please make sure to call zz_init before ' + 'using any logger methods!');
		}
...
observer.apply(null, [path]);
		...
		private function monitorTimer_tick(e : TimerEvent) : void {
			...
			g_lastFrames = 0;
			g_lastTimeStamp = now;
			g_gateway.invokeRemoteService('MonitoringService', 'trackFPS_memoryUse_timestamp', fps, System.totalMemory, now);
		}
	}
}
...
d:'debug', i:'info', n:'notice', w:'warning', e:'error', c:'critical', f:'fatal'
};
